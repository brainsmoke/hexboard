...
func NewRippleFilter(brightness float64, wave []float64, transform func(Vector2) Vector2, screen TextScreen) *RippleFilter {
...
var RippleAlt = []float64 {
    .333, .333,   1.,   1., .333, .333, .333, .333, .333, .333,
...
func (r *RippleFilter) Render(f *FrameBuffer, old *FrameBuffer, tick uint64) {
	...
	r.mutex.Lock()
	for j:=l-1; j >= 0; j-- {
		if r.ripples[j].count > r.ripples[j].cache.maxD + uint32(len(r.wave)) {
